...
"""}),
   ...
   notes=["element_type: solid(기본) | tshell | shell, direction: auto(기본, 자동 탐지) | x | y | z (+/- 부호도 같은 축). 모르는 값은 rc=1",
          ...
          "그 죽은 PID·지워진 요소(EID)·지워진 노드(NODE)를 가리키던 자리를 훑어 옮긴다 — 체적 의미로 쓰이는 *SET_PART_LIST/_TITLE/_COLUMN 은 층 PID 전부로(한 줄 8개 규칙을 지켜 줄을 늘린다), tied 계열 접촉(*CONTACT_*TIED*/*TIEBREAK*/*SPOTWELD*/*CONTACT_CONSTRAINT_)은 상대측 기하를 적층 축에 투영해 층이 하나로 정해질 때만 그 층으로, 그 밖의 접촉(AUTOMATIC·ERODING·SINGLE_SURFACE)은 층 전부를 담은 새 세트로(STYP 3→2)",
          "한 세트를 tied 와 체적 소비자가 함께 쓰면 세트를 복제해 가른다(새 SID 를 발급). 층이 애매하거나 shell 층이 섞이면 옮기지 않고 이유만 보고한다",
