...
def main():
    ...
    # ── R2b: '_TITLE' 카드의 제목 줄·데이터 줄 ────────────────────────────
    # 데이터 줄이 없으면 쓸 수 없는 카드라 rc=1 이다. 반면 제목 줄만 빠진 카드는 사용자가 실제로
    # 이렇게 쓰므로 거절하지 않는다 — 어셈블러가 그 줄을 데이터 줄로 읽고 [WARN] 을 낸 뒤
    # 빠진 제목 줄을 층 제목으로 채워 내보낸다(그래야 LS-DYNA·자체 리더가 그 재질을 읽는다).
    print("[R2b] '*MAT_..._TITLE' 에 데이터 줄이 없으면 거절하고, 제목 줄만 없으면 채워서 통과한다")
    TITLE_ONLY = "*MAT_ELASTIC_TITLE\nTitle\n$#     mid        ro         e        pr"
    both_paths(binary, "title_only",
               "  - thickness: 1.0\n    material_card: |\n" + indent_block(TITLE_ONLY, 6),
               expect_rc=1, want_msg="no data line")
    NO_TITLE1 = ("*MAT_ELASTIC_TITLE\n"
                 "$#     mid        ro         e        pr\n"
                 "        90  7.85E-09  2.10E+05       0.3")
    NO_TITLE2 = NO_TITLE1.replace("        90", "        91")
    both_paths(binary, "no_title",
               "  - thickness: 1.0\n    title: Substrate\n    material_card: |\n" + indent_block(NO_TITLE1, 6) +
               "  - thickness: 1.0\n    title: Cover\n    material_card: |\n" + indent_block(NO_TITLE2, 6),
               expect_rc=0, want_msg="had no title line")
