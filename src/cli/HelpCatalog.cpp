...
void printOverview(std::ostream& os, const std::string& version) {
    ...
    os << "  - 명령줄에 준 경로만 작업 폴더 기준. assemble·strip·단독 op 명령(convert/offset/update/warpage…)은\n";
    os << "    YAML 안의 상대 경로(model/base_model/output/dat_file/dynain…)를 그 YAML 파일이 있는 폴더 기준으로\n";
    os << "    푼다 — 작업 폴더로 되돌아가지 않는다\n";
    os << "    (KooRemapper strip cfg/strip.yaml 의 output: ../data/box_stripped.k → cfg/../data/box_stripped.k)\n";
    os << "  - 예외: load/boundary/contact/relax/database/implicit/modal/ale/cclip/matdb 는 폴더 없는 이름(box.k)만\n";
    os << "    YAML 폴더 기준으로 풀고, 폴더가 붙은 상대 경로(../data/box.k)는 아직 작업 폴더에서 찾는다\n";
