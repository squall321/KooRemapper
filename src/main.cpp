...
// 기존 영문 상세 help (op 별 인자·YAML 형식). 모르는 op 면 false
static bool printLegacyHelp(ConsoleOutput& console, const std::string& helpCmd) {
    if (helpCmd == "map") {
        ...
        console.println("  material:                  # optional E, nu (default: the part's *MAT_ELASTIC)");
        console.println("    E: 210000");
        console.println("    nu: 0.3");
        console.println("  operations:");
        console.println("    - type: bend");
        console.println("      target_pid: 1            # 0 or omitted = all parts");
        console.println("      plane: xy                # xy | yz | zx  (x1,x2 = X,Y | Y,Z | Z,X)");
        console.println("      mode: deform             # deform: move nodes + reverse stress | stress: stress only");
        console.println("      source: formula          # formula | dat | dat_pair");
        console.println("      expression: \"0.001*x1\"   # source: formula -> deflection w(x1, x2)");
        console.println("      # source: dat      -> dat_file: deflection.dat");
        console.println("      # source: dat_pair -> dat_top: top.dat  +  dat_bottom: bottom.dat");
        std::cout << "\n";
        ...
        std::cout << "\n";
        console.println("Deflection grid file (dat):");
        console.println("  Whitespace-separated matrix mapped onto the part's in-plane bounding box.");
        console.println("  Column 0 = x1 min, last column = x1 max; row 0 = x2 MAX, last row = x2 min.");
        console.println("  Values are deflections in model length units.");
        ...
        console.println("Punch outline: polygon or spline (closed loop of >= 3 points).");
        std::cout << "\n";
        console.println("YAML Config Format:");
        console.println("  base_model: flat.k");
        ...
        console.println("      target_pid: 1");
        console.println("      plane: xy                # xy | yz | zx");
        console.println("      direction: -z            # +z | -z | +x | -x | +y | -y (punch direction)");
        console.println("      depth: 0.5               # > 0 indent, < 0 emboss (must be non-zero)");
        console.println("      r1: 2.0                  # bottom-side transition arc radius (> 0)");
        console.println("      r2: 0.5                  # surface-side transition arc radius (> 0)");
        console.println("      stress: true             # compute prestress");
        console.println("      shape:");
        console.println("        type: polygon          # polygon | spline");
        console.println("        points:                # flat-bottom outline, model coordinates (x1, x2) of the plane");
        console.println("          - [0, 0]");
        console.println("          - [5, 0]");
        console.println("          - [5, 5]");
        console.println("          - [0, 5]");
        std::cout << "\n";
        console.println("Notes:");
        console.println("  Inside the outline the part is pushed flat by depth; outside it returns to the");
        console.println("  surface through an r1 arc then an r2 arc (transition width r1 + r2).");
        console.println("  shell_thickness  Shell stress thickness (0 = from *SECTION_SHELL)");
        ...
        console.println("Reads a measured warpage grid (dat) and deforms the mesh or writes prestress.");
        std::cout << "\n";
        console.println("YAML Config Format:");
        console.println("  base_model: flat.k");
        ...
        console.println("      target_pid: 1");
        console.println("      dat_file: warpage.dat     # measured deflection grid (relative to the YAML folder)");
        console.println("      plane: xy                 # xy | yz | zx");
        console.println("      deflection_axis: +z       # +z | -z | +x | -x | +y | -y");
        console.println("      unit: um                  # um (default) | mm | m  (unit of the grid values)");
        console.println("      mode: prestress           # prestress (default, initial stress) | deform (move nodes)");
        console.println("      morph_factor: 1.0         # deflection scale (> 0)");
        console.println("      finite_strain: true       # true: von Karman (default) | false: Kirchhoff");
        console.println("      outside_behavior: zero    # zero (default) | clamp | extrapolate");
        console.println("      mask_value: 9999          # grid value treated as missing (interpolated)");
        console.println("      # data_bbox:              # grid extent in plane coordinates (default: part bbox)");
        console.println("      #   x_min: 0.0");
        console.println("      #   x_max: 100.0");
        console.println("      #   y_min: 0.0");
        console.println("      #   y_max: 100.0");
        std::cout << "\n";
        console.println("Notes:");
        console.println("  - dat file: whitespace-separated matrix (rows x columns of deflection values),");
        console.println("    column 0 = plane axis-1 min, row 0 = plane axis-2 MIN (bend dat uses row 0 = max)");
        console.println("  - Bilinear interpolation between grid points");
...
// op 상세 = 한글 요약·사용법 + 영문 상세(있으면) + 검증된 사례·주의
static void printOpHelp(ConsoleOutput& console, const help::OpHelp& op) {
    help::printHeader(std::cout, op);
    std::cout << "\n";
    std::cout.flush();
    printLegacyHelp(console, op.name);
    std::cout.flush();
    help::printExample(std::cout, op);
    std::cout.flush();
}

// KooRemapper help [<op> | <검색어...> | all]
static int runHelp(ConsoleOutput& console, int argc, char* argv[]) {
    if (argc <= 2) {
        help::printOverview(std::cout, VERSION);
        return 0;
    }
    std::string query;
    ...
    if (query == "all") {
        help::printOverview(std::cout, VERSION);
        for (const auto& op : help::catalog()) {
            std::cout << "\n";
            printOpHelp(console, op);
        }
        return 0;
    }
    if (const auto* op = help::find(query)) {
        printOpHelp(console, *op);
        return 0;
    }
    auto hits = help::search(query);
    if (hits.size() == 1) {
        printOpHelp(console, *hits[0]);
        return 0;
    }
    if (hits.empty()) {
        console.error("No op matches '" + query + "'. Run 'KooRemapper help' for the list.");
        return 1;
    }
    std::cout << "'" << query << "' 검색 결과 " << hits.size() << "건 — KooRemapper help <op> 로 상세\n\n";
    for (const auto* op : hits) {
        std::string n = op->name;
        n.resize(std::max<size_t>(n.size(), 16), ' ');
        std::cout << "  " << n << " " << op->summary << "   [" << op->category << "]\n";
    }
    return 0;
}
...
static int runMain(int argc, char* argv[]) {
    ...
    if (command == "help" || command == "--help" || command == "-h") {
        return runHelp(console, argc, argv);
    }

    // <op> --help / -h
    if (argc == 3 && (std::string(argv[2]) == "--help" || std::string(argv[2]) == "-h")) {
        if (const auto* op = help::find(command)) {
            printOpHelp(console, *op);
            return 0;
        }
    }
