...
$dirs   = @("rubber_static_lv1","rubber_static_lv2","rubber_static_lv3","rubber_static_lv4",
             "rubber_static_lv5","rubber_static_lv6","rubber_static_lv7","rubber_static_lv8")
$base   = "D:\KooRemapper\tests\integration"
$results = @()
foreach ($dir in $dirs) {
    $runDir = "$base\$dir"
    $inp    = "$runDir\input.k"
    if (-not (Test-Path $inp)) { Write-Host "[$dir] SKIP: input.k not found" -ForegroundColor Yellow; continue }
    ...
    $proc = Start-Process -FilePath $lsdyna -ArgumentList "i=input.k memory=256m ncpu=1" `
        ...
        -RedirectStandardOutput "$runDir\stdout.txt" -RedirectStandardError "$runDir\stderr.txt" -Wait
    ...
    if (Test-Path "$runDir\messag") {
        ...
        foreach ($ln in $lines) {
            if ($ln -match "(?i)(error|warning.*fatal|\*\*\* Error)" -and
                $ln -notmatch "error tolerance|error norm|convergence error|relative error") { $errors += $ln.Trim() }
        }
    }
    ...
    foreach ($e in ($errors | Select-Object -Unique | Select-Object -First 5)) { Write-Host "  >> $e" -ForegroundColor Red }
