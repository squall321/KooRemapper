...
- ⚠ **dev 감독자가 2026-09-19 20:20 이후 멈춰 있었다.** 이 검토 중 우리 쪽에서 프로세스를 죽인 것이고,
  되살리려 했으나 이 세션의 권한으로는 상주 프로세스를 띄울 수 없었다. 그 서버에서 아래 한 줄이면 된다
  (고친 `install-autostart.sh` 가 크론까지 매분 감시로 바꿔 준다).
  ```
  bash platform/infra/scripts/install-autostart.sh
  ```
