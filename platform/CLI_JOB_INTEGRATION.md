...
- [ ] `infra/apptainer/cli.def` — 바이너리 + gmsh + material_db 만 담은 자체완결 이미지
- [ ] `infra/scripts/build-cli.sh` — cli.sif 빌드
- [ ] `jobs/` — SLURM 예제 + README (원샷 `apptainer exec/run`)
...
- [ ] KooRemapper를 pyKooCAE `occProject/Generators` 스타일의 모듈로 감싸는 래퍼
- [ ] CLI subprocess 방식(체인 스텝 친화, 상태 의존 없음) + 카탈로그 재사용
- 통합점: Runner/체인의 한 스텝으로 op 실행.

## (C) `kooremapper-core` 설치형 패키지
- [ ] `argbuild` + `catalog`(catalog_data.json 포함)를 독립 pip 패키지로 추출
- [ ] CLI·MCP·모듈이 한 소스(op 스키마 45개)를 공유
