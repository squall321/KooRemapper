...
예전에 조용히 기본값으로 떨어지던 값들이 지금은 오류로 끝나고 산출물도 안 나온다.
`describe_operation(op)` 의 `keys[].values` 를 그대로 따르라. 자주 틀리는 것들.

- `restack` 의 `element_type`(층별 포함): `solid` / `tshell` / `shell` 만. 대소문자까지 맞춰야 한다(`SOLID` 도 거절).
